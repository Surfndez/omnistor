...
class SetEntryMark
{
	...
	private static $api = '/fsentry/setentrymark/';// Service api
	 ...
	 * */	
	function payload($isfolder, $entryid, $markid)
	{			
		$dom = new DOMDocument('1.0');
		$dom->encoding = 'UTF-8';
		$root = $dom->createElement('setentrymark');// The root element of request payload 
		...
		$item = $dom->createElement('entryid', $entryid);
				$root->appendChild($item);
		$item = $dom->createElement('markid', $markid);
				$root->appendChild($item);
		$xmlStr = $dom->saveXML();
	...
	function parse($xml) 
	{
		...
		{	
			...
			{
				case 'status':   		 $SetEntryMarkResponse['status'] = $child; break;				
			}	 
		}
