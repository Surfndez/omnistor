<?php
class Deletesharecode
{
	private $sid;// It has been provided from Creative Cloud Web	
	private static $api = '/fsentry/deletesharecode/';// Service api
	 ...
	 * */	
	function payload($entrytype, $entryid, $password)
	{	
		$password_md5 = md5(strtolower($password));// The deletesharecoded file's password, it must require using lower-case letters and encrypt the password to its MD5 hash
		
		$dom = new DOMDocument('1.0');
		$dom->encoding = 'UTF-8';
		$root = $dom->createElement('getsharecode');// The root element of request payload 
				$dom->appendChild($root);
		$item = $dom->createElement('token', $this->token);
				$root->appendChild($item);
		$item = $dom->createElement('userid', $this->userid);
				$root->appendChild($item);
		$item = $dom->createElement('entrytype', $entrytype);
				$root->appendChild($item);	
		$item = $dom->createElement('entryid', $entryid);
				$root->appendChild($item);		
		$item = $dom->createElement('password', $password_md5);
				$root->appendChild($item);
		$xmlStr = $dom->saveXML();
	...
	function parse($xml) 
	{
		...
		{	
			...
			{
				case 'status':   		 $DeletesharecodeResponse['status'] = $child; break;
				case 'scrip':   		 $DeletesharecodeResponse['scrip'] = $child; break;
			}	 
		}
