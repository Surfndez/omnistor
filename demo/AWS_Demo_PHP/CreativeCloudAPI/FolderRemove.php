...
class FolderRemove
{
	...
	private static $api = '/folder/remove/';// Service api
	 ...
	 * */	
	function payload($id, $ischildonly)
	{		
		...
		$item = $dom->createElement('id', $id);
				$root->appendChild($item);	
		$item = $dom->createElement('ischildonly', $ischildonly);
				$root->appendChild($item);
		$xmlStr = $dom->saveXML();
	...
	function parse($xml) 
	{
		...
		{	
			...
			{
				case 'status':   		 $FolderRemoveResponse['status'] = $child; break;
			}	 
		}	
