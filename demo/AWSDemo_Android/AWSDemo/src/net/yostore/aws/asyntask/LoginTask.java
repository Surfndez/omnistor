...
//    		MessageDialog.show(ctx, ctx.getString(R.string.app_name), msg.toString());
		...
		if ( apiCfg.mySyncFolderId == null || apiCfg.mySyncFolderId.trim().length() == 0 )
		{
			...
			/*
			FindPropfindHelper profind = new FindPropfindHelper(apiCfg.SYNCROOTID, apiCfg.SYNCFOLDERNAME, "system.folder");
			try
			{
				FindPropfindResponse rsp = (FindPropfindResponse)profind.process(apiCfg);
				if ( rsp.getStatus() != APIException.GENERAL_SUCC )
				{
					msg.append("Find sync folder fail, status:").append(rsp.getStatus());
					Log.e(TAG, msg.toString());
			...
			{
				msg.append("Find sync folder error:").append(e.getMessage());
				Log.e(TAG, msg.toString(), e);
