...
import entity.FolderRemoveResponse;

public class FolderRemove 
{	

	static String API = "/folder/remove/";// Service api
	
	public static void main(String[] args) throws Exception
	{	
		FolderRemove folderRemove = new FolderRemove();
		FolderRemoveResponse rsp = folderRemove.getResponse();		
		System.out.println(rsp.toString());
	}

	public FolderRemoveResponse getResponse() throws Exception
	{
		...
		String id = "44922569";// Removing the specify folder's id
		String ischildonly = "0"; // 0 = Delete this folder | 1 = Delete the child folder of this folder
		
		String[] elmName = { "token", "userid", "id", "ischildonly" };// Define each XML tag name
		String[] data = { token, userInfo.userid, id, ischildonly };// Set each value of tag
		...
		// Compose the payload
		FolderRemoveResponse rsp = new FolderRemoveResponse();
		int j;
		...
		{
			throw new Exception("Error : You have to check FolderRemove api response");
		}
		
		System.out.println(" --- FINISH REMOVE FOLDER --- \n");
