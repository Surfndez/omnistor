...
import entity.FolderCreateResponse;

public class FolderCreate 
{	
	
	static String API = "/folder/create/";// Service api
	
	public static void main(String[] args) throws Exception
	{	
		FolderCreate folderCreate = new FolderCreate();
		FolderCreateResponse rsp = folderCreate.getResponse();		
		System.out.println(rsp.toString());
	}

	public FolderCreateResponse getResponse() throws Exception
	{
		 ...
		 * */		
		String root = "create";// The root element of request payload 			
		String parentFolderID = "44812268";// Parent folder's id
		String isencrypted = "0";
		String NewFolder="NewFolder";
		
		String displayName = new sun.misc.BASE64Encoder().encode( NewFolder.getBytes());// Creating the specify folder's name, it needs to be encode by Base64		

		String attribute = "<creationtime>1313054123</creationtime><lastaccesstime>1313054123</lastaccesstime><lastwritetime>1313054123</lastwritetime>"; //Folder's attribute context
		
		String[] elmName = { "token", "userid", "parent", "isencrypted", "display", "attribute" };// Define each XML tag name
		String[] data = { token, userInfo.userid, parentFolderID, isencrypted, displayName, attribute };// Set each value of tag
		...
		// Compose the payload
		FolderCreateResponse rsp = new FolderCreateResponse();
		int j;
		...
		{
			throw new Exception("Error : You have to check FolderCreate api response");
		}
		
		System.out.println("THE NEW FOLDER'S ID IS : " + rsp.getId());
